...
export function SignupForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const { error } = await createAccount(name.trim(), normalizeEmail(email), password)
      if (error) throw error
      setIsError(false)
      setFeedback("Conta criada. Verifique seu e-mail antes de entrar.")
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      <FieldGroup>
        {feedback && <div role="alert" className={isError ? "text-sm text-destructive" : "text-sm text-muted-foreground"}>{feedback}</div>}
        ...
        <Field><Button type="submit" disabled={isSubmitting} className="w-full">{isSubmitting ? AUTH_COPY.signup.submitting : AUTH_COPY.signup.submit}</Button></Field>

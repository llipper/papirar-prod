  ...
  return (
    <DashboardShell title="Home" description="Seu espaço de estudo jurídico.">
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(130px,.55fr)_minmax(130px,.55fr)_minmax(220px,.8fr)]">
        <Card className="relative min-h-48 overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-br from-amber-500/10 via-amber-500/5 to-transparent dark:from-amber-500/15 dark:via-card dark:to-card shadow-none">
          <CardContent className="relative z-10 flex min-h-48 flex-col justify-between p-6">
            <div className="max-w-sm">
              <p className="text-xs font-semibold text-muted-foreground">Bom estudo, {firstName}.</p>
              <h1 className="mt-2 font-heading text-2xl font-black tracking-tight text-foreground sm:text-3xl">Continue de onde você parou</h1>
              <p className="mt-2 text-sm leading-6 text-muted-foreground">{progress ? `Retome ${progress.law_title} e avance no seu ritmo.` : "O conhecimento te aproxima dos seus objetivos."}</p>
            </div>
            <Button asChild size="sm" className="w-fit rounded-full">
              <Link href={continueHref}>{progress ? "Continuar leitura" : "Abrir biblioteca"} <ArrowRight /></Link>
            </Button>
          </CardContent>
          <div className="absolute right-3 bottom-0 hidden h-40 w-40 rotate-6 overflow-hidden rounded-t-[2rem] border-4 border-background/60 bg-card shadow-xl sm:block">
            <Image src={continuedBook?.coverPath ?? "/capas/constituicao_federal.png"} alt="Livro em destaque" fill sizes="160px" className="object-cover" />
          </div>
        </Card>
        ...
        <Card className="min-h-48 overflow-hidden rounded-2xl border border-border/60 bg-card shadow-none">
          <CardContent className="flex min-h-48 flex-col justify-between p-5">
            <div>
              ...
              <p className="mt-5 text-xs font-medium text-muted-foreground">Progresso de leitura</p>
              <p className="mt-1 text-lg font-bold leading-6 text-foreground truncate">{progress ? progress.law_acronym : "Seu estudo começa aqui"}</p>
              <p className="mt-1 text-xs text-muted-foreground truncate">{progress ? studyTime(progress.total_seconds) : "Escolha uma lei e comece agora."}</p>
            </div>
            <Button asChild size="sm" variant="outline" className="w-fit rounded-full">
              <Link href={continueHref}>{progress ? "Continuar" : "Abrir biblioteca"} <ArrowRight /></Link>
            </Button>
          </CardContent>
        </Card>
      </section>

      <DashboardSection title="Acesso rápido" description="Tudo o que você usa para estudar.">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {quickLinks.map(({ title, description, href, icon: Icon }) => (
            <Link key={href} href={href} className="group">
              <Card className="h-full rounded-2xl border border-border/60 bg-card shadow-none transition-all group-hover:-translate-y-0.5 group-hover:bg-muted/60">
      ...
      <DashboardSection title="Sua biblioteca" description="Acesse rapidamente os textos disponíveis." href="/dashboard/biblioteca">
              ...
              <Card className="h-full rounded-2xl border border-border/60 bg-card shadow-none transition-all group-hover:-translate-y-0.5 group-hover:bg-muted/60">
                <CardContent className="flex items-center gap-3 p-3">
                  <div className="relative size-11 shrink-0 overflow-hidden rounded-xl bg-muted">
                    <Image src={book.coverPath ?? "/capas/constituicao_federal.png"} alt="" fill sizes="44px" className="object-cover" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      <Badge variant="secondary" className="max-w-20 truncate">{book.acronym}</Badge>
                      <span className="text-[10px] text-muted-foreground">{book.updatedAt}</span>
                    </div>
                    <h3 className="mt-1 truncate font-heading text-sm font-bold text-foreground">{book.title}</h3>
...
function MetricCard({ icon: Icon, label, value, detail }: { icon: typeof BookOpen; label: string; value: string; detail: string }) {
  return (
    <Card className="min-h-48 rounded-2xl border border-border/60 bg-card shadow-none">
      <CardContent className="flex min-h-48 flex-col justify-between p-5">
        <div className="flex size-9 items-center justify-center rounded-xl bg-muted text-foreground">
          <Icon className="size-4" />
        </div>
        <div>
          ...
          <p className="mt-1 font-heading text-3xl font-black tracking-tight text-foreground">{value}</p>
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h2 className="font-heading text-lg font-black text-foreground">{title}</h2>
          <p className="text-sm text-muted-foreground">{description}</p>
        </div>
        {href ? (
          <Button asChild variant="ghost" size="sm">
            <Link href={href}>Ver tudo <ArrowRight /></Link>
          </Button>
        ) : null}
      </div>
      {children}
